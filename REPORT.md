...
3. **太陽位置**: 本レポートでは (Az, El) = (90°,30°), (180°,70°), (270°,30°) と簡略化したが、`pedestal_tilt/sun.py`(`sun_altaz`)で観測地・日時から厳密計算できる(NOAA/Meeus 系、標準ライブラリのみ、誤差 ~0.01° 級、ΔT・大気差を考慮)。利用例は `examples/demo_sun.py`。
...
### 8.7 実機の機構:3本リニアアクチュエータ(パラレル機構)

写真の実機は §8.2 の Az–El–XEl 回転ジンバルではなく、**中央支点まわりに張った3本の
可変長アクチュエータ(蛇腹シリンダ)** の伸縮で皿を指向する**パラレル機構**である
(`pedestal_tilt/actuator_tripod.py`)。

- **逆運動学(閉形式)**:home(中立)で皿は天頂を向く。指向 (a, h) は天頂から θ=90−h を方位 a へ傾けた姿勢 `R = tilt_rotation_matrix(90−h, a)`(§2 と同じ回転)で表し、皿側取付点 `Pᵢ = O + R·pᵢ`、基台側 `Bᵢ`(固定)として脚長 `Lᵢ = |Pᵢ − Bᵢ|`。
- **ワークスペース**:各脚長は `[Lmin, Lmax]` のみ実現可能 → 到達範囲は**天頂まわりの円錐(cap)**。全方位 360° は脚長配分で届くが低仰角はストロークで頭打ち。代表寸法(H=1.0, rb=0.6, rp=0.35, stroke=[0.80,1.30])では **β ≈ 45°(仰角 45° 以上・全方位)**。
- 回転ジンバルの**天頂キーホール**(§8.1)はここでは生じず、代わりに**円錐状ワークスペース＋パラレル特異点**という別種の限界になる。全天・低仰角には、この上に方位回転台などの追加段が必要。

検証は `tests/test_pedestal_tilt.py::TestActuatorTripod`、3D シムは `sim/tripod3d.html`(脚長 vs 時間グラフ・到達円錐・寸法スライダー、起動時に脚長 golden を Python と突き合わせ)。
...
- ~~太陽位置の正確計算~~ → **実装済み**(`pedestal_tilt/sun.py`, 標準ライブラリのみ。サブ秒角が必要なら astropy 連携も可)
