...
| File | Description |
|------|-------------|
| `__init__.py` | 公開 API のバレル(2軸: `Observation`, `fit_tilt`, `correction`, `forward_pointing`, `image_offset`, `tilt_rotation_matrix` / 3軸: `forward_3axis`, `ik_3axis`, `ik_3axis_hold`, `plan_pass`, `max_azimuth_step` / 可動域: `JointLimits`, `within_limits`, `pass_fits_limits`, `peak_abs_xel` / パラレル機構: `TripodGeometry`, `leg_lengths`, `reachable`, `max_zenith_distance` 他 / 太陽位置: `sun_altaz`, `refraction_deg`, `delta_t_seconds`) |
...
| `sun.py` | 観測地・日時 → 太陽の **(方位, 高度)**。`sun_altaz`(NOAA/Meeus 系, stdlib のみ, 誤差~0.01°級, ΔT・大気差を考慮), `refraction_deg`(Sæmundsson), `delta_t_seconds`(Espenak&Meeus)。時刻系は太陽位置=TT・恒星時=UT。`demo.py` の太陽位置ハードコードを置換できる。 |
